...
class FileGenerator {
    ...
    // MARK: - Per-message output (Zomato fork)

    /// Generates one output `.swift` file per top-level proto message.
    /// Enums and proto extensions each get their own file too.
    /// Returns (filename, contents) pairs — one per output file.
    func generateOutputFilesPerMessage(
        ...
        var results: [(filename: String, contents: String)] = []
        ...
        // One .swift file per top-level message
        for (descriptor, message) in zip(fileDescriptor.messages, messages) {
            var p = CodePrinter(addNewlines: true)
            generatePerFileHeader(printer: &p)

            message.generateMainStruct(printer: &p, parent: nil, errorString: &errorString)
            if errorString != nil { return [] }
            ...
            if !protoPackage.isEmpty {
                p.print("", "fileprivate let _protobuf_package = \"\(protoPackage)\"")
            }
            message.generateRuntimeSupport(printer: &p, file: self, parent: nil)

            results.append((filename: "\(descriptor.name).pb.swift", contents: p.content))
        }

        // One .swift file for all top-level enums (if any)
        if !enums.isEmpty {
            var p = CodePrinter(addNewlines: true)
            generatePerFileHeader(printer: &p)
            for e in enums {
            ...
            let baseName = splitPath(pathname: fileDescriptor.name).base
            results.append((filename: "\(baseName)_enums.pb.swift", contents: p.content))
        }

        // One .swift file for proto extensions (if any — usually empty for combined.proto)
        if !extensionSet.isEmpty {
            var p = CodePrinter(addNewlines: true)
            generatePerFileHeader(printer: &p)
            let pathParts = splitPath(pathname: fileDescriptor.name)
            p.print(
                "",
                "// MARK: - Extension support defined in \(pathParts.base + pathParts.suffix)."
            )
            extensionSet.generateMessageSwiftExtensions(printer: &p)
            extensionSet.generateFileProtobufExtensionRegistry(printer: &p)
            extensionSet.generateProtobufExtensionDeclarations(printer: &p)
            results.append(
                (filename: "\(pathParts.base)_extensions.pb.swift", contents: p.content)
            )
        }
    ...
    /// Shared file header for every per-message output file:
    /// DO NOT EDIT banner + imports + version check.
    private func generatePerFileHeader(printer p: inout CodePrinter) {
        p.print(
            """
            // DO NOT EDIT.
            // swift-format-ignore-file
